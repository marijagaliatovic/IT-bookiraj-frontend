...
const AccomodationItem = (item: accomodation) => {
  return (
    <Link href={`/apartmentlistings/${item.apartmentId}`} className=" top-0 w-full xl:w-1/4 lg:w-1/3 ml:w-1/4 rounded-md md:w-1/2 bg-white flex flex-col justify-around items-center lg:m-4 my-4 hover:shadow-2xl">
  ...
  <div className="bg-white relative basis-6/12 w-8/12 m-4 flex flex-col justify-between gap-1 text-center">
    ...
    <div className="flex flex-col self-center justify-center m-3">
      ...
      <div className="flex flex-col text-center bg-white">
        ...
        <p className={`bg-white self-center font-serif font-medium not-italic tracking-wide ${item.specialOffer !== 0 ? "line-through" : ""}`}>
